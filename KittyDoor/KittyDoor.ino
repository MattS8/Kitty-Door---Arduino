...
void setup_remote_debugging() {
  ...
  if (Firebase.RTDB.get(&fbRecvData, "debug/kitty_door/msg_count")) {
    ...
    Serial.println(debugValues.logCounter);
  } else {
...
void read_hardware_override() {
  ...
  if (newForceOpen != values.forceOpen || newForceClose != values.forceClose) {
    ...
    status.desired = values.forceOpen == LOW
                       ? STATUS_OPEN
...
bool print_debug_message(const String& message) {
  if (!debug_message.equals(message)) {
...
void operate_on_door() {
  ...
  if (values.forceOpen == LOW) {
    if (values.upSense == HIGH) {
      if (status.desired == STATUS_OPEN) {
        print_debug_message("Force Open Enabled:\n\tOpening Door");
        open_door();
      } else {
        print_debug_message("Force Open Enabled:\n\tWARNING: Door was already opened but is not reading as opened!");
        stop_door_motors(true);
      }
    } else {  // Door is already open
  ...
  // If enabled and door is not down, close door
    ...
    if (values.downSense == HIGH) {
      if (status.desired == STATUS_CLOSED) {
        print_debug_message("Force Close Enabled:\n\tClosing Door");
        close_door();
      } else {
        print_debug_message("Force Close Enabled:\n\tWARNING: Door was already closed but is not reading as closed!");
        stop_door_motors(true);
      }
    } else {  // Door is already closed
  ...
  // Else, check if we are in automatic mode or remote override mode
    ...
    debug_msg = "Remote Override Mode Enabled!";
    Serial.println("   ");
    if (status.desired == STATUS_CLOSED) {  // Remote command to CLOSE door
      if (values.downSense == HIGH) {       // Door is not closed yet
        debug_msg += "\n\tClosing Door";
    ...
    } else if (status.desired == STATUS_OPEN) {  // Remote command to OPEN door
      if (values.upSense == HIGH) {              // Door is not open yet
        Serial.println("   Opening Door...");
        open_door();
      } else {  // Door is already open
    ...
    } else {  // Bad state, should never reach this
  ...
  // If in effect, check to see if it is time to open the door
    ...
    if (millis() > values.delayOpening) {
      ...
      } else {  // It got dark before delay time was over!
  ...
  // If in effect, check to see if it is time to close the door
    ...
    if (millis() > values.delayClosing) {
      ...
      } else {  // It got light before delay time was over!
